
              
              </td>
            </tr>
          </table>
        </td>
        <td width="20">&nbsp;</td>
      </tr>
      <tr> 
        <td width="20"><img src="images/spacer.gif" alt="" width="20" height="10"></td>
        <td><img src="images/spacer.gif" width="20" height="10"></td>
        <td width="20"><img src="images/spacer.gif" alt="" width="20" height="10"></td>
      </tr>
    </table>
    <!--Start Search/Clear Buttons-->
      <table border="0" cellspacing="0" cellpadding="0" width="800" align="right">
        <tr> 
          <td width="80%" align="right">
               <input class="button2a" type="button" value="Release"  onclick="setCurrentReferralAction('executeRelease');"/>
	  </td> 
          <td nowrap width="5">&nbsp;</td>
         <td align="left">
               <input class="button2a" type="button" value="Cancel"  onclick="setCurrentReferralAction('cancel');"/>
         </td>               
        </tr>
  </table>
    <!--End Criteria-->