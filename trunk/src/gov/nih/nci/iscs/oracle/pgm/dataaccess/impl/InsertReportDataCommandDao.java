...
public class InsertReportDataCommandDao extends ActionCommandDao implements InsertReportDataCommand {
    ...
    public Object execute(String mSessionId, Long mReportId, Long mApplId, 
                          String mAction, String oUserId) {
        ...
        try {
            if (mAction.equalsIgnoreCase(DELETE_ACTION)) {
                ...
                mUpdatePassed = 
                        insertData(mSessionId, mReportId, mApplId, mConnection);
            }
        } catch (CommandDaoException ex) {
            throw new CommandDaoException("Accept Referral Failed!!!" + 
                                          ex.toString());
        } catch (SQLException ex) {
            throw new CommandDaoException("Accept Referral Failed!!!" + 
                                          ex.toString());
        } finally {
            SessionFactoryUtils.closeSessionIfNecessary(mSession, 
                                                        getSessionFactory());
        }
    ...
    public boolean insertData(String mSessionId, Long mReportId, Long mApplId, 
                              Connection mConnection) throws CommandDaoException, 
                                                             SQLException {
        /*
     ...
     */
        ...
        String mQueryString = 
            "{call REPORTS_UTL_PKG.STORE_TEMP_IDS(?,?,?,?,?)}";
        try {
    ...
    public boolean deleteData(String mSessionId, Long mReportId, 
                              Connection mConnection) throws CommandDaoException, 
                                                             SQLException {
        /*
     * Call the stored procedure to insert temp data for report
     ...
     */
        ...
        String mQueryString = 
            "delete from REPORT_TEMP_GRANTS_T where SESSION_ID = ? and REPORT_ID = ?";
        try {
